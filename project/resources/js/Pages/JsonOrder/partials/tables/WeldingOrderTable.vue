...
const isUserHasFullPermissions = props.auth.permissions.includes(`full_welding_orders_permission`);
const isUserHasMediumPermissions = props.auth.permissions.includes(`medium_welding_orders_permission`);
    ...
    <div v-for="machine in machines">
        ...
        <div v-for="(order, orderIndex) in props.order">
            ...
            <div v-show="!monitoringMode"
                ...
                <PasteTaskButton
                    v-if="isTaskInLocalStorage['welding'] && isUserHasFullPermissions"
...
{'text-lg' : monitoringMode}"
                        ...
                        <p class="w-3/12 text-center text-gray-700" :class="monitoringMode ? 'text-md' : 'text-sm'">
                            {{ order.PRODUCT_TITLE }}</p>
                        <p class="w-1/12 text-center" :class="monitoringMode ? 'text-lg' : 'tтext-sm'">
                            {{ order.PLAN_PLEN_WELD_KG ?? '' }}</p>
                        <p class="w-4/12 italic text-center" :class="monitoringMode ? 'text-md' : 'text-sm'">
                            {{ order.NOTES }}</p>

                        <p v-if="order.STATUS !== 'Finished'" class="w-1/12 text-right text-sm text-gray-500 ml-auto">
            ...
            <div class="flex flex-col p-6">
                <section class="text-gray-800">
                    ...
                    <div v-if="isEditedOrder && isUserHasFullPermissions && orderForm.FINISHED_AT"
                        ...
                        <span class="absolute top-10 text-xs text-orange-800 left-48">
                            * Изменяйте время выполнения ответственно и только в крайних случаях
                        </span>
                    </div>
                </section>
            </div>
