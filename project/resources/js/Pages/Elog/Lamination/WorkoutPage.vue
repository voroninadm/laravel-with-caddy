...
/**
 * Суммирование показателей простоев
 * @param task
...
<template>
    <MachinesReportFilter
        :header="'Выработка ламинация'"
        :machines="machines"
        :workers="workers"
        :routeToSubmit="'lamination.workout'"
        :auth="props.auth"
    ...
    <div v-if="props.tasks && tasks.length > 0" class="flex flex-col mt-4 h-full overflow-auto text-center">
        <table class="mx-4 mb-20 printable-table" ref="printableTable">
            ...
            <tr v-for="(task, index) in tasks"
                ...
                <td class="border" title="Операторы">
                ...
                <td class="border" title="Материал">{{ task.mat2_id }}</td>
                <td class="border" title="Материал">{{ task.mat3_id }}</td>
