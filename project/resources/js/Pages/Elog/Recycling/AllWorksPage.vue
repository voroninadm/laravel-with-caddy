...
import {isOkSvgShow, summarize} from "@/common/elogTaskHelper.js";
...
const destroyTask = () => {
    router.delete(`/recycling/destroy/${taskToDelete.value}`);
...
const isUserCanChange = () => {
    return props.auth.permissions.includes('medium_recycling_permission') || props.auth.permissions.includes('full_recycling_permission')
};

//=== rules
const isUserCanEditDelete = (taskWorkDate) => {
    if ((props.auth.permissions.includes('medium_recycling_permission'))) {
    ...
    } else if (props.auth.permissions.includes('full_recycling_permission') ) {
        return true
    }
}
</script>
...
<template>
    <MachinesReportFilter
        :header="'Вся переработка'"
        :machines="machines"
        :workers="workers"
        :nomenclatures="nomenclatures"
        :routeToSubmit="'recycling.allWorks'"
        :localStorageKey="'recyclingAllWorks'"
        :auth="props.auth"
        :is-excel-button-disabled="props.tasks === null"
        :excel-path-to-convert="'/recycling/allWorks'"
    />
    ...
    <div v-if="props.tasks && tasks.length > 0" class="flex flex-col mt-4 h-full overflow-auto text-center">
        <table class="mx-4 mb-20">
            ...
            <tr class="text-sm bg-gray-100 sticky top-0 z-20 opacity-100">
                ...
                <th  v-if="isManagerCanSee()" class="border px-2">Машинист</th>
                ...
                <th class="border px-2">Номенклатура гранулята</th>
                <th class="border px-2">Выработка кг</th>

                <th v-if="isManagerCanSee()" class="border px-2">Номенклатура литника</th>
                <th v-if="isManagerCanSee()" class="border px-2">Масса литников</th>
                ...
                <th v-if="isManagerCanSee()" class="border px-2">Замена ножей</th>
                <th v-if="isManagerCanSee()" class="border px-2">Замена резцов</th>
                <th v-if="isManagerCanSee()" class="border px-2">Мойка машины</th>
                <th v-if="isManagerCanSee()" class="border px-2">Чистка</th>
            ...
            <tr v-for="(task, index) in tasks"
                ...
                :class="{ 'bg-slate-100/40': index % 2 === 1,
                 ...
                 'bg-red-200/40' : Boolean(task.prepare_ok) === false && task.prepare_ok != null}">
                    ...
                    <Dropdown :open="false">
                        ...
                        <template #content>
                            <div v-if="isUserCanEditDelete(task.created_at)">
                                <DropdownLink :href="`/recycling/edit/${task.id}`">
                                    Изменить
                                </DropdownLink>
                                <button
                ...
                <td v-if="isManagerCanSee()" class="border" title="Машинист">{{ task.machinist_id }}</td>
                <td v-if="isManagerCanSee()" class="border" title="Подсобный рабочий">{{ task.helper_id }}</td>
                ...
                <td class="border" title="Тип гранулята">{{ task.nomenclature }}</td>
                <td class="border bg-yellow-200/50" title="Выработка кг">{{ task.workout_mass }}</td>

                <td v-if="isManagerCanSee()" class="border" title="Тип литника">{{ task.ingots_type }}</td>
                <td v-if="isManagerCanSee()" class="border" title="Масса литников">{{ task.waste_mass }}</td>
                ...
                <td v-if="isManagerCanSee()" class="border" title="Замена ножей">{{ task.change_knifes }}</td>
                <td v-if="isManagerCanSee()" class="border" title="Замена резцов">{{ task.change_cutter }}</td>
                <td v-if="isManagerCanSee()" class="border" title="Мойка машины">{{ task.clean_machine }}</td>
                <td v-if="isManagerCanSee()" class="border" title="Чистка машины">{{ task.tech_clean }}</td>
            ...
            <tr class="bg-gray-100 text-gray-600">
                ...
                <td v-if="isManagerCanSee()" class="border" title="Машинист"></td>
                ...
                <td class="border" title="Тип гранулята"></td>
                <td class="border bg-yellow-200/50" title="Выработка кг">{{ summarize(tasks, 'workout_mass') }}</td>

                <td v-if="isManagerCanSee()" class="border" title="Тип литника"></td>
                <td v-if="isManagerCanSee()" class="border" title="Масса литников">{{ summarize(tasks, 'waste_mass') }}</td>
                ...
                <td v-if="isManagerCanSee()" class="border" title="Замена ножей">{{ summarize(tasks, 'change_knifes') }}</td>
                <td v-if="isManagerCanSee()" class="border" title="Замена резцов">{{ summarize(tasks, 'change_cutter') }}</td>
                <td v-if="isManagerCanSee()" class="border" title="Мойка машины">{{ summarize(tasks, 'clean_machine') }}</td>
                <td v-if="isManagerCanSee()" class="border" title="чистка машины">{{ summarize(tasks, 'tech_clean') }}</td>
