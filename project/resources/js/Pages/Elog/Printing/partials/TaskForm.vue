...
const form = useForm({
    machine_id: props.printTask ? props.printTask.machine_id : '',
    work_date: props.printTask ? props.printTask.work_date : new Date(),
    work_shift: props.printTask ? props.printTask.work_shift : setWorkShiftBasedOnTime(8, 20),
    master_id: props.printTask ? props.printTask.master_id : '',
    operator1_id: props.printTask ? props.printTask.operator1_id : '',
    operator2_id: props.printTask ? props.printTask.operator2_id : '',
    operator3_id: props.printTask ? props.printTask.operator3_id : '',
    operator_helper: props.printTask ? props.printTask.operator_helper : '',
    tkn: props.printTask ? props.printTask.tkn : '',
    work_plan: props.printTask ? props.printTask.work_plan : null,
    work_start: props.printTask ? props.printTask.work_start : '',
    work_finish: props.printTask ? props.printTask.work_finish : '',
    work_fact: props.printTask ? props.printTask.work_fact : null,
    work_productive: props.printTask ? props.printTask.work_productive : null,
    customer: props.printTask ? props.printTask.customer : '',
    print_title: props.printTask ? props.printTask.print_title : '',
    circulation: props.printTask ? props.printTask.circulation : null,

    mat1_id: props.printTask ? props.printTask.mat1_id : '',
    mat1count_plan: props.printTask ? props.printTask.mat1count_plan : null,
    mat1count: props.printTask ? props.printTask.mat1count : null,
    width1: props.printTask ? props.printTask.width1 : null,
    thickness1: props.printTask ? props.printTask.thickness1 : null,
    colors: props.printTask ? props.printTask.colors : null,
    workout_mass: props.printTask ? props.printTask.workout_mass : null,
    workout_length: props.printTask ? props.printTask.workout_length : null,
    workout_m2: props.printTask ? props.printTask.workout_m2 : null,
    otk_mass: props.printTask ? props.printTask.otk_mass : null,
    waste_plan: props.printTask ? props.printTask.waste_plan : null,
    waste_print: props.printTask ? props.printTask.waste_print : null,
    waste_raw: props.printTask ? props.printTask.waste_raw : null,
    waste_sum: props.printTask ? props.printTask.waste_sum : null,
    prepare_mass: props.printTask ? props.printTask.prepare_mass : null,
    prepare_hours: props.printTask ? props.printTask.prepare_hours : null,
    correction_PN: props.printTask ? props.printTask.correction_PN : null,
    correction_CMYK: props.printTask ? props.printTask.correction_CMYK : null,
    electro: props.printTask ? props.printTask.electro : null,
    mechanical: props.printTask ? props.printTask.mechanical : null,
    form_glue: props.printTask ? props.printTask.form_glue : null,
    service_replacement: props.printTask ? props.printTask.service_replacement : null,
    tech_service: props.printTask ? props.printTask.tech_service : null,
    short_downtime: props.printTask ? props.printTask.short_downtime : null,
    total_downtime: props.printTask ? props.printTask.total_downtime : null,
    no_reason_downtime: props.printTask ? props.printTask.no_reason_downtime : null,
    speed: props.printTask ? props.printTask.speed : null,
    no_human: props.printTask ? props.printTask.no_human : null,
    no_work: props.printTask ? props.printTask.no_work : null,
    no_raw: props.printTask ? props.printTask.no_raw : null,
    diff_circulation: props.printTask ? props.printTask.diff_circulation : null,
    notes: props.printTask ? props.printTask.notes : '',
    is_idle: props.printTask ? Boolean(props.printTask.is_idle) : false,
    is_complete: props.printTask ? Boolean(props.printTask.is_complete) : false,
});

const submit = () => {
    if (form.is_complete && (!form.work_productive && !form.total_downtime)) {
        showNotification('info', 'Для закрытия карты заполните необходимые поля, получите простои из Энкоста')
    }
        ...
        if (props.printTask) {
            form.patch(route('printing.update', props.printTask.id), {
                onSuccess: () => {
...
const IS_ADMIN = props.auth.permissions.includes('full_printing_permission');
...
const sendEncostRequest = async () => {
    ...
    try {
        ...
        const {
            ...
            'Формы': form_glue = null,
            'Замена расходных материалов': service_replacement = null,
            'Неисправность электрика': electro = null,
            'Неисправность механика': mechanical = null,
            'Корректировка пантонов': correction_PN = null,
            'Корректировка смук': correction_CMYK = null,
...
watch(
    [() => form.waste_print, () => form.waste_raw],
    ([newPrint, newRaw], [oldPrint, oldRaw]) => {
        if (newPrint !== oldPrint || newRaw !== oldRaw) {
            form.waste_sum = calculateWaste(form.waste_print, form.waste_raw)
        }
    }
);
    ...
    (newNumber, oldNumber) => {
        if (form.tkn !== '') {
            getTknDataFromDb(newNumber, oldNumber, (data) => {
                ...
                form.mat1count_plan = data.circulation_kg;
                form.work_plan = +data.printing_time.toFixed(2);
            });
        }
    }
);
...
watch(() => form.is_idle, (newValue, oldValue) => {
    if (newValue) {
        form.work_start = form.work_finish = form.mat1_id = '';

        form.work_plan = form.work_fact = form.mat1count_plan =
            form.mat1count = form.width1 = form.thickness1 = form.colors = form.workout_mass = form.workout_length =
                form.workout_m2 = form.otk_mass = form.diff_circulation = form.speed = null;
    }
});
</script>
...
<template>
    <div class="h-full gradient">
        <h1 class="ml-36 font-semibold text-md flex text-gray-600">
            ...
            <span class="ml-6">{{ printTask ? 'Изменение задания ' : 'Новое задание ' }} печати</span>
        </h1>
        ...
        <form @submit.prevent="submit">
            <article class="grid grid px-4 gap-8 mt-5 relative"
                ...
                <fieldset class="flex flex-col p-2 gap-2 rounded-lg shadow-2xl text-base bg-white">
                    ...
                    <div class="flex gap-4">
                        ...
                        <div class="flex flex-col items-center m-auto">
                            ...
                            <VueDatePicker v-model="form.work_date" day-picker :format="'dd.MM.yyyy'" locale="ru"
                                           ...
                                           :min-date="!props.auth.permissions.includes('full_printing_permission') ? calculateMinDatepickerDate() : null"
                                           auto-apply required/>
                    ...
                    <div class="flex items-center">
                        <input-label class="w-1/2" for="machine_id">Машина печати
                            <span class="text-red-600">*</span>
                        </input-label>
                        ...
                        <SelectInput :options="machines" v-model="form.machine_id" id="machine_id" required
                                     ...
                                     :placeholder-option="'выберите машину печати'"
                        />
                    </div>
                    ...
                    <div class="flex items-center">
                        <input-label class="w-1/2" for="operator_helper">Помощник оператора</input-label>
                        <TextInput
                            id="operator_helper"
                            class="h-6 block w-full"
                            v-model="form.operator_helper"
                            autocomplete="off"
                            list="workers"
                        />
                        <InputError class="mt-2" :message="form.errors.operator_helper"/>
                    </div>
                    ...
                    <div class="grid grid-cols-3 gap-4 mt-3">
                        ...
                        <div class="flex gap-4 items-center text-sm text-gray-600">
                            ...
                            >
                                    {{
                                    (form.total_downtime + form.work_productive) === 0 ? '' : (form.total_downtime + form.work_productive).toFixed(2)
                                }}
                                </span>
                        </div>
                    </div>
                ...
                <fieldset class="flex flex-col p-2 gap-2 rounded-lg shadow-2xl text-base bg-white relative">
                    <legend class="mt-2 w-full text-center font-semibold text-md mb-2">🧻 Расход материала</legend>

                    <div class="flex gap-2 pb-6">
                        <div class="w-full gap-2 flex flex-col ">
                            <input-label class="text-center" for="mat1_id">Материал</input-label>
                            <TextInput
                                id="mat1_id"
                                class="h-6 block w-1/2 m-auto"
                                :class="{'bg-gray-200': form.is_idle}"
                                :disabled=" form.is_idle"
                    ...
                    <div class="grid grid-cols-2 gap-10">
                        <div class="flex pb-6">
                            <div class="flex w-full gap-2 flex-col">
                                <input-label class="text-center" for="workout_mass">Выработка</input-label>
                                <div>
                                    <NumberInput
                                        id="workout_mass"
                                        class="h-6 block w-full text-sm"
                        ...
                        <div class="flex pb-6">
                            ...
                            <div class="flex w-full gap-2 flex-col">
                                <input-label class="self-center" for="waste_plan">Килограммы отходов</input-label>
                                <div>
                                    <NumberInput
                                        id="waste_plan"
                                        class="h-6 block w-full text-sm"
                                ...
                                <div>
                                    <NumberInput
                                        id="waste_plan"
                                        class="h-6 block w-full text-sm"
                                        v-model.number="form.prepare_mass"
                                        placeholder="... приправка"
                                        title="Приправка, килограмм"
                                        step="0.01"
                                    />
                                    <InputError class="mt-2" :message="form.errors.prepare_mass"/>
                                </div>
                                <div>
                                    <NumberInput
                                        class="h-6 block w-full text-sm"
                                ...
                                <div>
                                    <NumberInput
                                        class="h-6 block w-full text-sm"
                                        v-model.number="form.waste_raw"
                                        placeholder="... сырья"
                                        title="сырья, кг"
                                        step="0.01"
                                    />
                                    <InputError class="mt-2" :message="form.errors.waste_raw"/>
                                </div>
                                <div>
                                    <NumberInput
                                        class="h-6 block w-full text-sm bg-gray-200"
                    ...
                    <div class="grid grid-cols-2 gap-10">
                        <div class="flex flex-col items-center">
                            <input-label for="colors">Краски, шт</input-label>
                            <NumberInput
                                id="colors"
                                class="h-6 block w-full"
                        ...
                        <div class="flex flex-col items-center">
                            <input-label for="speed">Скорость печати</input-label>
                            <NumberInput
                                id="speed"
                                class="h-6 block w-full text-sm"
                    ...
                    <div>
                        <div class="flex flex-col items-center pt-4">
                            <input-label class="text-center" for="notes">Примечание</input-label>
                            <TextareaInput
                                id="notes"
                ...
                <Transition enter-active-class="animate__animated animate__fadeInDown">
                    <fieldset v-show="form.is_complete"
                        ...
                        <legend class="mt-2 w-full text-center font-semibold text-md mb-3">⚒ Технические операции и
                            простои
                        </legend>
                        ...
                        <div class="w-full gap-4 grid grid-cols-2">
                            ...
                            <div class="flex gap-2 items-center">
                                <input-label class="w-4/5" for="prepare_mass">Приправка</input-label>
                                <NumberInput
                                    id="prepare_mass"
                                    class="h-6 block w-full text-sm"
                        ...
                        <div class="w-full gap-4 grid grid-cols-2">
                            <div class="flex gap-2 items-center">
                                <input-label class="w-4/5" for="form_glue">Переклейка форм</input-label>
                                <NumberInput
                                    id="form_glue"
                                    class="h-6 block w-full text-sm"
                                    v-model.number="form.form_glue"
                                    placeholder="... часов"
                                    ...
                                    :class="{'bg-gray-200' : !IS_ADMIN}"
                                />
                                <InputError class="mt-2" :message="form.errors.form_glue"/>
                            </div>
                            <div class="flex gap-2 items-center">
                                <input-label class="w-4/5" for="service_replacement">Замена расходников</input-label>
                                <NumberInput
                                    id="service_replacement"
                                    class="h-6 block w-full text-sm"
                                    v-model.number="form.service_replacement"
                                    placeholder="... часов"
                                    title="Замена расходных материалов, часов"
                                    step="0.01"
                                    :readonly="!IS_ADMIN"
                                    :class="{'bg-gray-200' : !IS_ADMIN}"
                                />
                                <InputError class="mt-2" :message="form.errors.service_replacement"/>
                            </div>
                        </div>
                        ...
                        <div class="w-full gap-4 grid grid-cols-2">
                            <div class="flex gap-2 items-center">
                                <input-label class="w-4/5" for="correction_PN">Коррекция пантонов</input-label>
                                <NumberInput
                                    id="correction_PN"
                                    class="h-6 block w-full text-sm"
                                    v-model.number="form.correction_PN"
                                    placeholder="... часов"
                                    title="ПКоррекция пантонов, часов"
                                    step="0.01"
                                    :readonly="!IS_ADMIN"
                                    :class="{'bg-gray-200' : !IS_ADMIN}"
                                />
                                <InputError class="mt-2" :message="form.errors.correction_PN"/>
                            </div>
                            <div class="flex gap-2 items-center">
                                <input-label class="w-4/5" for="correction_CMYK">Коррекция CMYK</input-label>
                                <NumberInput
                                    id="correction_CMYK"
                                    class="h-6 block w-full text-sm"
                                    v-model.number="form.correction_CMYK"
                                    placeholder="... часов"
                                    title="Коррекция CMYK, часов"
                                    step="0.01"
                                    :readonly="!IS_ADMIN"
                                    :class="{'bg-gray-200' : !IS_ADMIN}"
                                />
                                <InputError class="mt-2" :message="form.errors.correction_CMYK"/>
                            </div>
                        </div>
                        ...
                        <div class="flex flex-col items-center pt-2">
                            <input-label class="text-center" for="no">Часы отсутствия</input-label>
                            <div class="flex gap-4">
                                <NumberInput
                                    id="no"
                                    class="h-6 block w-full text-sm"
            ...
            <div class="flex justify-center gap-5 items-center w-full p-6">
                <span v-if="form.is_idle"
                      class="absolute italic w-max bottom-15 flex items-center gap-1 text-sm text-red-700 left-1/2 transform -translate-x-1/2">
