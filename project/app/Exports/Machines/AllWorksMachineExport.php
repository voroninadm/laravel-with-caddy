...
class AllWorksMachineExport implements FromView, WithStyles, ShouldAutoSize
{    use Exportable;
    ...
    {
        return view("ExcelExports/Elog/{$this->type}", [
            'data' => $this->data,
    ...
    public function styles(Worksheet $sheet)
    {
        return [
            ...
            // серый фон для контента первой колонки
            ...
            // стилизуем последнюю строку с "итого" - добавляем красный фон
            'A' . $sheet->getHighestRow() . ':' . $sheet->getHighestColumn() . $sheet->getHighestRow() => [
                'fill' => ['fillType' => Fill::FILL_SOLID, 'startColor' => ['argb' => 'FFFFCCCB']], // Red background
            ],
        ];
    }
